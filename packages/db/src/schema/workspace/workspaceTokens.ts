...
export const workspaceTokens = pgTable('workspace_tokens', {
  ...
  provider: workspaceExternalProviderType('provider').notNull().default(WorkspaceExternalProviderType.Slack),
  ...
  index('workspace_tokens_workspace_id_idx').on(table.workspaceId),
]);
